...
struct swl_desktop_icon_node {
	...
	char command[64];
	char label[32];
	struct wlr_scene_buffer *buf; /* pra reposicionar (mover) e esconder/mostrar */
	bool hidden;
};
...
struct swl_desktop *swl_desktop_create(struct wlr_scene_tree *parent, int top_offset) {
	...
	for (size_t i = 0; i < N_DEFAULT_ICONS; i++) {
		...
		desktop->icons[i].buf = buf;
		desktop->icons[i].hidden = false;
		snprintf(desktop->icons[i].label, sizeof(desktop->icons[i].label),
			"%s", default_icons[i].label);
...
const char *swl_desktop_hit_test(struct swl_desktop *desktop, double x, double y) {
	int idx = swl_desktop_hit_test_index(desktop, x, y);
	return idx >= 0 ? desktop->icons[idx].command : NULL;
}
...
const char *swl_desktop_icon_label(struct swl_desktop *desktop, int index) {
	...
	return desktop->icons[index].label;
}
...
void swl_desktop_move_icon(struct swl_desktop *desktop, int index, int x, int y) {
	...
	struct swl_desktop_icon_node *icon = &desktop->icons[index];
	icon->x = x;
	icon->y = y;
	wlr_scene_node_set_position(&icon->buf->node, x, y);
}

void swl_desktop_icon_pos(struct swl_desktop *desktop, int index, int *x, int *y) {
	if (index < 0 || index >= desktop->count) {
		return;
	}
	*x = desktop->icons[index].x;
	*y = desktop->icons[index].y;
}
...
void swl_desktop_hide_icon(struct swl_desktop *desktop, int index) {
	...
	struct swl_desktop_icon_node *icon = &desktop->icons[index];
	icon->hidden = true;
	wlr_scene_node_set_enabled(&icon->buf->node, false);
}
...
void swl_desktop_show_all_icons(struct swl_desktop *desktop) {
	for (int i = 0; i < desktop->count; i++) {
		...
		if (icon->hidden) {
			icon->hidden = false;
			wlr_scene_node_set_enabled(&icon->buf->node, true);
		}
	}
}
