...
/* Mesma coisa, com pango_attr_letter_spacing pra abrir espaço entre
 * letras — usado pelas variantes _tracked. spacing_px <= 0 não aplica
 * nada (equivalente a swl_make_layout puro). */
static PangoLayout *swl_make_layout_tracked(cairo_t *cr, const char *text,
	...
	if (spacing_px > 0) {
		...
		pango_attr_list_insert(attrs,
			pango_attr_letter_spacing_new((int)(spacing_px * PANGO_SCALE)));
...
void swl_draw_text_tracked(cairo_t *cr, const char *text, double x, double y,
		double size, const char *font, bool bold, double spacing_px) {
	PangoLayout *layout = swl_make_layout_tracked(cr, text, size, font, bold, spacing_px);
	cairo_move_to(cr, x, y);
	pango_cairo_show_layout(cr, layout);
	g_object_unref(layout);
}
