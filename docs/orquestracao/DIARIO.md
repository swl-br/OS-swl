...
(comportamento no QEMU). Aprovado pelo usuário e integrado; aviso
ATENDIDO.

## 2026-09-11 — Atalhos WM Alt+F4/F9/F10 (admin)

Veredito padrão: BOM; base ATUAL; build limpo (comportamento no
QEMU). Aprovado pelo usuário e integrado.

## 2026-09-11 — Alt+Tab no compositor (admin)

Veredito padrão: BOM (ciclo frente/trás, pula minimizadas, sem loop);
base ATUAL; build limpo (comportamento só validável no QEMU).
Aprovado pelo usuário e integrado com ida pro `main`.
...
## 2026-09-12 — Ctrl+Alt+T integrado (admin)

Veredito padrão: BOM; base ATUAL; build limpo (tecla no QEMU).
Aprovado pelo usuário e integrado; aviso ATENDIDO.
