...
| V1 | **Identidade visual do terminal** — `userland/swlfetch` + `userland/neo-face.txt` no repo (protótipo aprovado pelo usuário). Falta: splash automático ao abrir terminal (profile no rootfs), prompt `SWL:~$`, help, clear com cabeçalho, spinner/progresso como funções shell; versão nativa C/ASM do `swlfetch` como desdobramento. Ver IDEIAS "Identidade visual". | Pacote shell primeiro (zero risco); C nativo e boot depois. | — |
