...
| B1 | **Ícones PNG do desktop** — CONCLUÍDA em 2026-09-09: prova definitiva com PNG vermelho sólido (400 px vermelhos no r0c0 + screenshot) — pipeline `resolve→cairo→paint` funciona; serial mostra `status=0` nos 13. Aparência "fina" é tamanho da arte (ICON_GLYPH_SIZE/padding do PNG), não falha de carga. | — |
...
| C2 | **swlc: `extern` em `for`** | CONCLUÍDA (Buffy fez do jeito dele na v2.2–v2.16; fix do Grok ficou moot e foi descartado da entrada). | Buffy |
| C3 | **swlc: leitura de teclado** | CONCLUÍDA (Buffy: `swl_read(fd, buf, n)` existe e funciona; orquestrador verificou lendo stdin). | Buffy |
| V2 | **Novo visual das janelas (pedido do usuário, design completo)** — sofisticado/elegante: (1) cantos ARREDONDADOS leves (nada de quina pontuda, sem exagero) e barra fina; (2) menubar OCULTA por padrão: botão com seta-para-baixo no topo; ao apertar, menubar desliza de dentro do botão em sincronia e a seta vira (fecha ao contrário); (3) vidro FOSCO sutil (blur leve, não transparente) com transição pras cores do fundo em botões e áreas específicas; (4) interruptores arredondados estilo Apple; (5) fechar/maximizar/minimizar REDONDOS estilo Apple MANTENDO as cores atuais; (6) restrição: manter leveza/performance. TSWL é o piloto (menubar dele vira dropdown do botão). | — |
| K1 | **Política de config do kernel (auditoria 2026-09-10)** — .config atual tem 1041 builtins (defconfig de 94 linhas sobre defaults; bzImage 5,5MB). Base sã: M686, HZ_1000, PREEMPT_VOLUNTARY, DRM_BOCHS, DEVTMPFS, SMP, sem DEBUG. Gordura: SND/WLAN/FB/NETFILTER ligados, KALLSYMS_ALL, MODULES. Decisões pendentes: CPU (M686 compat vs bump), PREEMPT full, slim de drivers, MODULES on/off. Toda mudança exige rebuild + boot QEMU validando. | kernel (sem dono!) |
...
| T5 | **TSWL: seleção com mouse + copiar/colar** | CONCLUÍDA em 2026-09-09: pacote fix-t4-t5 (Grok) — dirty-row + barra T1 restaurada + seleção/highlight + forward decls; build limpo, T5/R-11/R-14 OK no sanitizer.  (duplo-clique/palavra pendente — aviso `2026-09-10-aviso-t5word.md`) (duplo-clique incluído 2026-09-10)  | tswl |
...
| S1 | **App Configurações (Claude)** | CONCLUÍDA (casca) em 2026-09-10: `apps/swlconfig/` integrado (12 seções navegáveis, só Sobre funcional; swlappkit p/ menu Arquivo/Ajuda; build 10/10 + sysinfo 8/8 no repo; screenshots). Demais seções por etapa. | Claude |
| W1 | **TSWL deforma no resize (PAUSADO, vai pro Claude em zip só do app)** — tentativas: clamp compositor, clamps/cliente, sem-menubar, sem-taskbar — deforma igual. Usuário decide: zip só do app p/ Claude resolver. | QEMU/teste. |
| W2 | **rootfs sem dados do libinput** — CONCLUÍDA em 2026-09-09: seção quirks no `build-gui-rootfs.sh` (mesmo padrão xkb/fonts, host como fallback). Verificado headless: warning sumiu, compositor no ar. | rootfs. |
| S2 | **swlsysinfo: janela/configure + CPU% (Grok)** — entrega `s2-swlsysinfo-files.tar.gz` revisada: delta CPU entre samples, configure com mínimo, surface acompanha; fix do primeiro quadro preservado; build 7/7 no repo; rodando no aninhado sem erro, aguardando olho do usuário (números mexem? cabe na tela?). | Grok | **SUSPENSA** (reformulação dos apps — ver aviso geral). |
