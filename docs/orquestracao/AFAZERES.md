...
| W1 | **Janela TSWL deforma no resize do output (larga e baixa)** — print QEMU 2026-09-10. Atualização: com fix W1 aplicado, persiste MAS só no TSWL (outros apps ok) → suspeito lado cliente (`toplevel_configure`/render/decor no tswl), não compositor. Aguardando print+log novos. | QEMU/teste. |
