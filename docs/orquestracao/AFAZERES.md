...
| M1 | **Barra de menu por app (Arquivo, Editar, Ver, Configurar…)** — padrão clássico dentro de cada janela, como todo sistema tem; hoje só o painel global tem menu. Componente pronto: `apps/swlappkit/` (integrada). Falta adotar no TSWL (rebase dos hunks sobre o `main` atual, mantendo T2+R-14 — ver aviso superado em `docs/revisao/2026-09-09-aviso-openhands-menubar-t2.md`). | Identidade desktop | Grok |
