...
| V1 | **Identidade visual do terminal (pacote shell, simples)** — com `userland/swlfetch` + `userland/neo-face.txt` já no repo, falta: splash automático ao abrir terminal (profile no rootfs), prompt `SWL:~$`, help bonito, clear com cabeçalho, loading/spinner/progresso como funções shell. Tudo texto, zero binário novo. Desdobramentos (não desta tarefa): `swlfetch` nativo C/ASM, boot com padrão OK, biblioteca C `swl_ui_*` (ver IDEIAS). | Pacote shell primeiro; resto depois. | — |
