...
| S2 | **swlsysinfo: janela/configure + CPU% (Grok)** — entrega `s2-swlsysinfo-files.tar.gz` revisada: delta CPU entre samples, configure com mínimo, surface acompanha; fix do primeiro quadro preservado; build 7/7 no repo; rodando no aninhado sem erro, aguardando olho do usuário (números mexem? cabe na tela?). | Grok |
