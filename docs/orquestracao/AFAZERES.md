...
| W1 | **Menubar: seleção travava e poluía cliques (CAUSA PROVADA)** — log QEMU com patch DBG mostrou `selecting=1` preso durante operação de menu (drag solto sobre a menubar; ramo do menu retornava sem desligar). Fix aplicado (`selecting=false` no ramo menubar) após teste no QEMU; validar no próximo boot. | tswl |
