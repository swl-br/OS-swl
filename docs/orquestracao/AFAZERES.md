...
| V2 | **Novo visual das janelas (time de apps)** — ENTROU 1ª leva 2026-09-11 (pedido do usuário): decorations.c (cantos arredondados, botões circulares macOS-order com cores SWL, tema claro/escuro) + theme.h (WARN + paleta light). Falta: menubar oculta c/ botão-seta, vidro fosco, toggles (especificado). defconfig novo junto (PREEMPT full, sandbox, zram — vale no próximo rebuild do kernel). | — |
