...
| T1 | **TSWL: cursor vira barra piscando no lugar certo** — hoje é bloco (ver `render.c:257-262`); usuário pede barra vertical piscando posicionada certo. | Pedido do usuário testando no QEMU. | tswl |
| T2 | **TSWL: scroll não rola** — Shift+PageUp/Down existe (`main.c:434-441`) mas usuário relata que não funciona no QEMU; wheel do mouse nem existe. Verificar Shift+PgUp no QEMU + implementar scroll por wheel. | Pedido do usuário testando no QEMU. | tswl |
