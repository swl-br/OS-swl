...
| W1 | **Menubar do TSWL: clica e não abre; resize seleciona sozinho** — print QEMU. Achados no código (orquestrador, sem executar): 1) `pointer_motion` atualiza hover SEMPRE (mesmo sem menu aberto e durante drags) e o desenho destaca `hover_menu` — highlight fantasma esperado; 2) clique press/release parece certo no papel — suspeita de entrega/evento durante grab do compositor ou coord. Verificar no QEMU: clicar Arquivo (abre?), mover p/ item + soltar (executa?), redimensionar com menu fechado (highlight mexe?). | QEMU/teste. |
