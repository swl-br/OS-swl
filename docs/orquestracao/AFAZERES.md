...
| W1 | **TSWL deforma no resize: geometria menubar/grid no cliente (GROK, PRIORITÁRIO)** — prints QEMU: janela larga + 1-2 linhas + farelos. Direcionamento: taskbar e clamp do compositor descartados; mirar `TSWL_MENUBAR_H` (reserva vs desenho), stride da cópia shm vs surface em transição, hover fantasma no resize. Base: branch mais novo (DEC-011). Corrigir logo. | Grok |
