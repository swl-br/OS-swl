...
| T1 | **TSWL: cursor vira barra piscando no lugar certo** — hoje é bloco (ver `render.c:257-262`); usuário pede barra vertical piscando posicionada certo. | Absorvida por T4 (terminal completo). | tswl |
| T2 | **TSWL: scroll não rola** — Shift+PageUp/Down existe (`main.c:434-441`) mas usuário relata que não funciona no QEMU; wheel do mouse nem existe. Verificar Shift+PgUp no QEMU + implementar scroll por wheel. | Absorvida por T4 (terminal completo). | tswl |
| T3 | **TSWL/QEMU: glifo █ (U+2588) vira tofu** — splash Neo em blocos fica ilegível no QEMU (cores OK, glifo falta); no host renderiza. Suspeita: fontconfig no rootfs mínimo não carrega `DejaVuSans.ttf` direito (só há ela em `gui-artifacts/fonts/`; TSWL pede mono). Verificar fonte efetiva no QEMU, adicionar mono com cobertura (ex. DejaVuSansMono) e preferir. Workaround atual: arte só-ASCII. | Sem isso o Neo bonito não aparece no sistema real. | tswl/gui |
| T4 | **TSWL terminal completo (dono a definir — alguém adota)** — cursor barra `|` piscando no lugar certo; mouse funcionando (clique + wheel/scroll); otimizado; texto reajusta ao resize; `swlfetch`/Neo auto-ajusta à largura; inclui T1+T2. Pedido do usuário testando no QEMU: "terminal completo". | Escopo grande, uma IA só de preferência (DEC-009). | — |
| M1 | **Barra de menu por app (Arquivo, Editar, Ver, Configurar…)** — padrão clássico dentro de cada janela, como todo sistema tem; hoje só o painel global tem menu. Definir componente reutilizável (hera de `menu.c`?) + adotar nos apps reais (TSWL, SWLPAD) primeiro. | Identidade desktop; depende do padrão de app (IDEIAS Plataforma). | — |
| S1 | **App Configurações completo (dono a definir)** — WiFi/rede, bluetooth, acessibilidade, apps, personalizar (tema, wallpaper), teclado+mouse, display, painel, notificações, sobre, idiomas, disco. Começa como casca com seções (algumas sem backend real, como a bandeja hoje) e ganha função por etapa. | Pedido do usuário: "algo completo". | — |
| S2 | **swlsysinfo: janela maior que a tela + CPU zerado + fantasma na borda** — app abre 640x400 fixo sem respeitar o configure (conteúdo cortado); CPU% tudo 0% (amostra inicial ou cálculo); possível cópia vazando na borda direita. App ainda untracked (`apps/swlsysinfo/`, teste do usuário); fix do primeiro quadro já feito pelo orquestrador. | Visto em print no aninhado. | — |
