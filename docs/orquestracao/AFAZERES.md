...
| W1 | **Janela TSWL deforma no resize (larga + 1-2 linhas + farelos)** — prints QEMU 2026-09-10. Isolado: SÓ no TSWL; taskbar descartada como causa (usuário). Suspeito atual: geometria menubar/grid no cliente — `TSWL_MENUBAR_H` (26px) reserva vs desenho, ou stride da cópia shm vs surface cairo em transição de resize (farelos = dessinc). Toggle taskbar revertido. | QEMU/teste. |
